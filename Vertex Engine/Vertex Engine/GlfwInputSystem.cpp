#include "pch.h"
#include "GlfwInputSystem.h"

VertexEngine::GlfwInputSystem::GlfwInputSystem(VertexEngine::GlWindow* _window)
{
	m_GameWindow = _window;
	InitProps();
}

bool VertexEngine::GlfwInputSystem::GetKey(KeyCode _key)
{
	return m_CurrentKeys[ConvertKeycodeToGLFW(_key)];
}

bool VertexEngine::GlfwInputSystem::GetKeyUp(KeyCode _key)
{
	return !m_CurrentKeys[ConvertKeycodeToGLFW(_key)] && m_PreviousKeys[ConvertKeycodeToGLFW(_key)];
}

bool VertexEngine::GlfwInputSystem::GetKeyDown(KeyCode _key)
{
	return m_CurrentKeys[ConvertKeycodeToGLFW(_key)] && !m_PreviousKeys[ConvertKeycodeToGLFW(_key)];
}

bool VertexEngine::GlfwInputSystem::GetMouse(KeyCode _key)
{
	return m_CurrentMouseKeys[ConvertKeycodeToGLFW(_key)];
}

bool VertexEngine::GlfwInputSystem::GetMouseDown(KeyCode _key)
{
	return m_CurrentMouseKeys[ConvertKeycodeToGLFW(_key)] && !m_PreviousMouseKeys[ConvertKeycodeToGLFW(_key)];
}

bool VertexEngine::GlfwInputSystem::GetMouseUP(KeyCode _key)
{
	return !m_CurrentMouseKeys[ConvertKeycodeToGLFW(_key)] && m_PreviousMouseKeys[ConvertKeycodeToGLFW(_key)];
}

bool VertexEngine::GlfwInputSystem::GetButton(KeyCode _key, int _joystick)
{
	return m_CurrentGamepadKeys[ConvertKeycodeToGLFW(_key)];
}

bool VertexEngine::GlfwInputSystem::GetButtonDown(KeyCode _key, int _joystick)
{
	return m_CurrentGamepadKeys[ConvertKeycodeToGLFW(_key)] && !m_PreviousKeys[ConvertKeycodeToGLFW(_key)];
}

bool VertexEngine::GlfwInputSystem::GetButtonUp(KeyCode _key, int _joystick)
{
	return !m_CurrentGamepadKeys[ConvertKeycodeToGLFW(_key)] && m_PreviousKeys[ConvertKeycodeToGLFW(_key)];
}

bool VertexEngine::GlfwInputSystem::IsGamepad(int _joystick)
{
	if (m_GameWindow)
		return glfwJoystickIsGamepad(_joystick);

	return false;
}

void VertexEngine::GlfwInputSystem::UpdateInputLogs()
{
	if (m_GameWindow == nullptr) return; // Do not update unless window handle has been asighned.

	// Update & poll keyboard inputs
	for (int i = 0; i < GLFW_KEY_LAST; i++) {
		m_PreviousKeys[i] = m_CurrentKeys[i];
		m_CurrentKeys[i] = glfwGetKey(m_GameWindow->GetWindowHandle(), i) == GLFW_PRESS;
	}

	// Update & poll mouse inputs
	for (int i = 0; i < GLFW_MOUSE_BUTTON_LAST; i++) {
		m_PreviousMouseKeys[i] = m_CurrentMouseKeys[i];
		m_CurrentMouseKeys[i] = glfwGetMouseButton(m_GameWindow->GetWindowHandle(), i) == GLFW_PRESS;
	}

	// Update & poll gamepad inputs
	if (IsGamepad(0)) {
		GLFWgamepadstate gamepadCurrentState;
		if (glfwGetGamepadState(GLFW_JOYSTICK_1, &gamepadCurrentState)) {
			for (int i = 0; i < 15; i++) {
				m_PreviousGamepadKeys[i] = m_CurrentGamepadKeys[i];
				m_CurrentGamepadKeys[i] = gamepadCurrentState.buttons[i] == GLFW_PRESS;
			}
		}
	}
}

int VertexEngine::GlfwInputSystem::ConvertKeycodeToGLFW(KeyCode _key)
{
	// Go through & return the matching keycode to the glfw version.
	switch (_key) {
		//Keyboard Letters
	case KeyCode::A: return GLFW_KEY_A;
	case KeyCode::B: return GLFW_KEY_B;
	case KeyCode::C: return GLFW_KEY_C;
	case KeyCode::D: return GLFW_KEY_D;
	case KeyCode::E: return GLFW_KEY_E;
	case KeyCode::F: return GLFW_KEY_F;
	case KeyCode::G: return GLFW_KEY_G;
	case KeyCode::H: return GLFW_KEY_H;
	case KeyCode::I: return GLFW_KEY_I;
	case KeyCode::J: return GLFW_KEY_J;
	case KeyCode::K: return GLFW_KEY_K;
	case KeyCode::L: return GLFW_KEY_L;
	case KeyCode::M: return GLFW_KEY_M;
	case KeyCode::N: return GLFW_KEY_N;
	case KeyCode::O: return GLFW_KEY_O;
	case KeyCode::P: return GLFW_KEY_P;
	case KeyCode::Q: return GLFW_KEY_Q;
	case KeyCode::R: return GLFW_KEY_R;
	case KeyCode::S: return GLFW_KEY_S;
	case KeyCode::T: return GLFW_KEY_T;
	case KeyCode::U: return GLFW_KEY_U;
	case KeyCode::V: return GLFW_KEY_V;
	case KeyCode::W: return GLFW_KEY_W;
	case KeyCode::X: return GLFW_KEY_X;
	case KeyCode::Y: return GLFW_KEY_Y;
	case KeyCode::Z: return GLFW_KEY_Z;
		// Keyboard numbers
	case KeyCode::Num0: return GLFW_KEY_0;
	case KeyCode::Num1: return GLFW_KEY_1;
	case KeyCode::Num2: return GLFW_KEY_2;
	case KeyCode::Num3: return GLFW_KEY_3;
	case KeyCode::Num4: return GLFW_KEY_4;
	case KeyCode::Num5: return GLFW_KEY_5;
	case KeyCode::Num6: return GLFW_KEY_6;
	case KeyCode::Num7: return GLFW_KEY_7;
	case KeyCode::Num8: return GLFW_KEY_8;
	case KeyCode::Num9: return GLFW_KEY_9;
		// Numpad
	case KeyCode::Numpad0: return GLFW_KEY_KP_0;
	case KeyCode::Numpad1: return GLFW_KEY_KP_1;
	case KeyCode::Numpad2: return GLFW_KEY_KP_2;
	case KeyCode::Numpad3: return GLFW_KEY_KP_3;
	case KeyCode::Numpad4: return GLFW_KEY_KP_4;
	case KeyCode::Numpad5: return GLFW_KEY_KP_5;
	case KeyCode::Numpad6: return GLFW_KEY_KP_6;
	case KeyCode::Numpad7: return GLFW_KEY_KP_7;
	case KeyCode::Numpad8: return GLFW_KEY_KP_8;
	case KeyCode::Numpad9: return GLFW_KEY_KP_9;
	case KeyCode::NumpadAdd: return GLFW_KEY_KP_ADD;
	case KeyCode::NumpadEqual: return GLFW_KEY_KP_EQUAL;
	case KeyCode::NumpadDecimal: return GLFW_KEY_KP_DECIMAL;
	case KeyCode::NumpadDivide: return GLFW_KEY_KP_DIVIDE;
	case KeyCode::NumpadEnter: return GLFW_KEY_KP_ENTER;
	case KeyCode::NumpadMultiply: return GLFW_KEY_KP_MULTIPLY;
	case KeyCode::NumpadSubtract: return GLFW_KEY_KP_SUBTRACT;
		// Functions
	case KeyCode::F1: return GLFW_KEY_F1;
	case KeyCode::F2: return GLFW_KEY_F2;
	case KeyCode::F3: return GLFW_KEY_F3;
	case KeyCode::F4: return GLFW_KEY_F4;
	case KeyCode::F5: return GLFW_KEY_F5;
	case KeyCode::F6: return GLFW_KEY_F6;
	case KeyCode::F7: return GLFW_KEY_F7;
	case KeyCode::F8: return GLFW_KEY_F8;
	case KeyCode::F9: return GLFW_KEY_F9;
	case KeyCode::F10: return GLFW_KEY_F10;
	case KeyCode::F11: return GLFW_KEY_F11;
	case KeyCode::F12: return GLFW_KEY_F12;
	case KeyCode::F13: return GLFW_KEY_F13;
	case KeyCode::F14: return GLFW_KEY_F14;
		// Symbols
	case KeyCode::Apostraphe: return GLFW_KEY_APOSTROPHE;
	case KeyCode::Backslash: return GLFW_KEY_BACKSLASH;
	case KeyCode::Slash: return GLFW_KEY_SLASH;
	case KeyCode::Comma: return GLFW_KEY_COMMA;
	case KeyCode::Period: return GLFW_KEY_PERIOD;
	case KeyCode::Minus: return GLFW_KEY_MINUS;
	case KeyCode::GraceAccent: return GLFW_KEY_GRAVE_ACCENT;
	case KeyCode::Semicolon: return GLFW_KEY_SEMICOLON;
	case KeyCode::LeftBracket: return GLFW_KEY_LEFT_BRACKET;
	case KeyCode::RightBracket: return GLFW_KEY_RIGHT_BRACKET;
		// Modifiers
	case KeyCode::LeftShift: return GLFW_KEY_LEFT_SHIFT;
	case KeyCode::RightShift: return GLFW_KEY_RIGHT_SHIFT;
	case KeyCode::LeftControl: return GLFW_KEY_LEFT_CONTROL;
	case KeyCode::RightControl: return GLFW_KEY_RIGHT_CONTROL;
	case KeyCode::LeftAlt: return GLFW_KEY_LEFT_ALT;
	case KeyCode::RightAlt: return GLFW_KEY_RIGHT_ALT;
	case KeyCode::LeftSuper: return GLFW_KEY_LEFT_SUPER;
	case KeyCode::RightSuper: return GLFW_KEY_RIGHT_SUPER;
	case KeyCode::CapsLock: return GLFW_KEY_CAPS_LOCK;
	case KeyCode::NumLock: return GLFW_KEY_NUM_LOCK;
	case KeyCode::ScrollLock: return GLFW_KEY_SCROLL_LOCK;
		//Gamepad Buttons
	case KeyCode::GamepadA: return GLFW_GAMEPAD_BUTTON_A;
	case KeyCode::GamepadB: return GLFW_GAMEPAD_BUTTON_B;
	case KeyCode::GamepadX: return GLFW_GAMEPAD_BUTTON_X;
	case KeyCode::GamepadY: return GLFW_GAMEPAD_BUTTON_Y;
	case KeyCode::GamepadBack: return GLFW_GAMEPAD_BUTTON_BACK;
	case KeyCode::GamepadGuide: return GLFW_GAMEPAD_BUTTON_GUIDE;
	case KeyCode::GamepadLeft: return GLFW_GAMEPAD_BUTTON_DPAD_LEFT;
	case KeyCode::GamepadRight: return GLFW_GAMEPAD_BUTTON_DPAD_RIGHT;
	case KeyCode::GamepadUp: return GLFW_GAMEPAD_BUTTON_DPAD_UP;
	case KeyCode::GamepadDown: return GLFW_GAMEPAD_BUTTON_DPAD_DOWN;
	case KeyCode::GamepadLeftBumper: return GLFW_GAMEPAD_BUTTON_LEFT_BUMPER;
	case KeyCode::GamepadRightBumper: return GLFW_GAMEPAD_BUTTON_RIGHT_BUMPER;
	case KeyCode::GamepadLeftTrigger: return GLFW_GAMEPAD_AXIS_LEFT_TRIGGER;
	case KeyCode::GamepadRightTrigger: return GLFW_GAMEPAD_AXIS_RIGHT_TRIGGER;
		// System
	case KeyCode::Escape: return GLFW_KEY_ESCAPE;
	case KeyCode::Enter: return GLFW_KEY_ENTER;
	case KeyCode::Spacebar: return GLFW_KEY_SPACE;
	case KeyCode::Tab: return GLFW_KEY_TAB;
	case KeyCode::Backspace: return GLFW_KEY_BACKSPACE;
	case KeyCode::Insert: return GLFW_KEY_INSERT;
	case KeyCode::Delete: return GLFW_KEY_DELETE;
	case KeyCode::Home: return GLFW_KEY_HOME;
	case KeyCode::End: return GLFW_KEY_END;
	case KeyCode::PageUp: return GLFW_KEY_PAGE_UP;
	case KeyCode::PageDown: return GLFW_KEY_PAGE_DOWN;
	case KeyCode::PrintScreen: return GLFW_KEY_PRINT_SCREEN;
	case KeyCode::Pause: return GLFW_KEY_PAUSE;
		// Arrow Keys
	case KeyCode::ArrowUp: return GLFW_KEY_UP;
	case KeyCode::ArrowDown: return GLFW_KEY_DOWN;
	case KeyCode::ArrowLeft: return GLFW_KEY_LEFT;
	case KeyCode::ArrowRight: return GLFW_KEY_RIGHT;
	}
}

void VertexEngine::GlfwInputSystem::InitProps()
{
	// Fill keyboard input states.
	std::fill(std::begin(m_CurrentKeys), std::end(m_CurrentKeys), false);
	std::fill(std::begin(m_PreviousKeys), std::end(m_PreviousKeys), false);

	// Fill mouse input states.
	std::fill(std::begin(m_CurrentMouseKeys), std::end(m_CurrentMouseKeys), false);
	std::fill(std::begin(m_PreviousMouseKeys), std::end(m_PreviousMouseKeys), false);

	// Fill gamepad input states.
	std::fill(std::begin(m_CurrentGamepadKeys), std::end(m_CurrentGamepadKeys), false);
	std::fill(std::begin(m_PreviousGamepadKeys), std::end(m_PreviousGamepadKeys), false);
}


